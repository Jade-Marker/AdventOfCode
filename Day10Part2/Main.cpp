...
int main()
{
	...
	CPU cpu;
	int cycleCount = 0;
	std::array<std::string, 6> output = { std::string(40, '.'), std::string(40, '.') , std::string(40, '.') , std::string(40, '.') , std::string(40, '.') ,std::string(40, '.') };
	...
	while (getline(file, line))
	{
		...
		{
			int modCount = cycleCount % 40;

			if (cpu.GetXRegister() > modCount - 2 && cpu.GetXRegister() < modCount + 2)
			{
				int outputY = cycleCount / 40;
				output[outputY][modCount] = '#';
			}
