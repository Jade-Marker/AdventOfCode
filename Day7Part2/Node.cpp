...
int Node::FindSmallestAboveNum(Node* node, int min)
{
	...
	for (auto iter = node->_children.begin(); iter != node->_children.end(); iter++)
	{
		if (!iter->_isFile)
		{
			if (iter->_size > min && iter->_size < currentSmallest)
				currentSmallest = iter->_size;

			int smallestInNode = Node::FindSmallestAboveNum((Node*)(&*iter), min);

			if (smallestInNode > min && smallestInNode < currentSmallest)
				currentSmallest = smallestInNode;
		}
	}
